...
use crate::tag_manager::id3::v2_3::utils::{encode_text_payload, create_header_with_version, to_synchsafe};

#[derive(Debug, Clone)]
pub struct V2_4 {}

fn from_synchsafe(bytes: [u8; 4]) -> usize { ((bytes[0] as usize) << 21) | ((bytes[1] as usize) << 14) | ((bytes[2] as usize) << 7) | (bytes[3] as usize) }

fn frame_from_synchsafe(bytes: [u8; 4]) -> usize {
    from_synchsafe(bytes)
}

fn frame_to_synchsafe(size: usize) -> [u8; 4] { to_synchsafe(size as u32) }
...
impl TagFormat for V2_4 {
    ...
    fn get_tags(&self, file_path: &PathBuf) -> std::io::Result<HashMap<FrameKey, TagValue>> {
        ...
        if file.read_exact(&mut header).is_err() { return Ok(HashMap::new()); }
        if &header[0..3] != b"ID3" || header[3] != 4 { return Ok(HashMap::new()); }
        ...
        while pos + 10 <= tag_data.len() {
            ...
            if id_bytes.iter().all(|b| *b == 0) { break; }
            let id = match std::str::from_utf8(id_bytes) {
                Ok(s) if s.trim().is_empty() => break,
                Ok(s) => s.to_string(),
                Err(_) => break
            };
            let size = frame_from_synchsafe([tag_data[pos + 4], tag_data[pos + 5], tag_data[pos + 6], tag_data[pos + 7]]);
            let _flags = &tag_data[pos + 8..pos + 10];
            if size == 0 || pos + 10 + size > tag_data.len() { break; }
            ...
            if id.starts_with('T') || id.starts_with('W') {
                if !content.is_empty() {
                    ...
                    let text = match encoding {
                        ...
                        0x01 => { if content[1..].starts_with(&[0xFF, 0xFE]) { String::from_utf16_lossy(&content[3..].chunks(2).filter(|c| c.len() == 2).map(|c| u16::from_le_bytes([c[0], c[1]])).collect::<Vec<_>>()) } else { "<Unsupported UTF-16>".to_string() } }
                ...
                if let Some(null_pos) = content[1..].iter().position(|&b| b == 0x00) {
                    ...
                    if pic_type_index >= content.len() { break; }
                    let _picture_type = content[pic_type_index];
                    let description_start = pic_type_index + 1;
                    let description_end = content[description_start..].iter().position(|&b| b == 0x00).map_or(content.len(), |p| description_start + p);
                    let image_data = if description_end < content.len() { &content[description_end + 1..] } else { &[] };
                    raw.insert(id, TagValue::Picture { mime: mime_type, data: image_data.to_vec() });
                }
            }
            pos += 10 + size;
        }
    ...
    fn write_tags(&self, file_path: &PathBuf, updated: HashMap<FrameKey, TagValue>) -> Result<(), ()> {
        use std::io::{Read, Seek, SeekFrom, Write};
        let mut file = OpenOptions::new().read(true).write(true).open(file_path).map_err(|_| ())?;
        let mut header = [0u8; 10];
        file.read_exact(&mut header).map_err(|_| ())?;
        if &header[0..3] != b"ID3" || header[3] != 4 { return Err(()); }
        let tag_size = from_synchsafe([header[6], header[7], header[8], header[9]]);
        let mut tag_data = vec![0u8; tag_size];
        ...
        for (k, v) in raw_updates {
            match v {
                ...
                TagValue::Picture { mime, data } => {
                    let encoded = crate::tag_manager::id3::v2_3::utils::encode_img_payload(&mime, 3, "", &data);
                    raw.insert(k.to_string(), encoded);
                }
            }
        }
        let frames_vec = raw.iter().map(|(id, content)| build_frame_v24(id, content)).collect::<Vec<_>>();
        ...
        for fr in frames_vec { id3_tag.extend_from_slice(&fr); }
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result { write!(f, "V2_4 Tag Format") }
}
